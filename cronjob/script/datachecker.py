...
def get_path(resource):
    try:
        resource_paths = {
            'database': lambda resource: "/".join([resource['database_connection'], resource['schema_name'], resource['table_name']]),
            'datalake': lambda resource: "/".join([resource['datalake_data']['container'], resource['datalake_data']['file_path']]),
    ...
    try:
        ...
        if resource['resource_type'] == 'datalake':
            ...
            result = datalake_service_client.find_file_or_directory(
                resource['datalake_data']['container'], resource['datalake_data']['file_path'])
        ...
        if len(package['resources']) > 0:
            for resource in package['resources']:
                ...
                if not result:
                    resource_data = {
                        'name': resource['name'],
                        'ckan_url': generate_ckan_url(package['dataset_name'], resource['id']),
...
def dispatch_workers(data, no_of_workers, target):
    result_list = []
    # partial_target = partial(target, log_queue)
    try:
        ...
        while not stop_api:
            try:
                response = requests.get(url, headers=headers, params={
                                        'rows': rows, 'start': start, 'include_private': True}, verify=False)
            except Exception as e:
                raise e
            ...
            if len(invalid_resources) > 0:
                for resource_info in invalid_resources:
                    email_content = generate_email_content(
                        resource_info['name'], resource_info['data_source_path'], resource_info['ckan_url'])
                    response = send_mail(
                        [resource_info['dataset_owner_email'], ENV['TEAMS_EMAIL']], email_content)
    except Exception as e:
        logger.error(e)
